...
void Scatter_Random::setRadius(fp_t radius) {
	this->radius = radius;
}


void Scatter_Random::setParallel(int8_t is_parallel) {
	this->is_parallel = is_parallel;
}


void Scatter_Random::setRaySize(uint8_t count) {
	this->count = count;
}


void Scatter_Random::scatter(RecordRay *src, MemoryControl_Scatter *memory_control) const {

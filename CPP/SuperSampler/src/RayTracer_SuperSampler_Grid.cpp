#include "../inc/RayTracer_SuperSampler_Grid.hpp"
...
// Operation Handling
void SuperSampler_Grid::setGridSize(int w, int h) {
	width	= w;
	height	= h;
...
Vec3f SuperSampler_Grid::sample(const Vec3f &point, const Vec3f &region, int depth, Vec3f(*get_color)(double x, double y, void *info), void *info) {
	// get point list
	...
	for (int x = 0; x < width; x++) {
		for (int y = 0; y < height; y++) {
			point_list[index] = 
			Vec3f(-w_half + x * w_pixel,  -h_half + y * h_pixel, 0);
			index++;
		}
	}

...
__device__ static inline bool	refract		(Vec3f &refracted, const Vec3f &incident, const Vec3f &normal, fp_t index_from, fp_t index_to);


// Operation Handling
__device__ Scatter_Refraction::Scatter_Refraction() {
...
__device__ void Scatter_Refraction::scatter(RecordRay *src, MemoryControl_Scatter *memory_control) const {
...
// index_out:	ray from
// index_in:	ray to
__device__ bool Scatter_Refraction::setRecord_outer(RecordRay *dst, RecordRay *src, fp_t *index_from, fp_t *index_to) const {
	// variable preparation

...
// Operation Handling
// hitable
__device__ Hitable_Sphere::Hitable_Sphere() {
	updateBoundingBox();
}
...
__device__ void Hitable_Sphere::setCenter(const Vec3f &center) {
	this->origin	= center;
	this->center	= center;
	updateBoundingBox();
}
...
__device__ void Hitable_Sphere::setRadius(fp_t radius) {
	this->radius	= radius;
	updateBoundingBox();
}


__device__ int8_t Hitable_Sphere::hit(RecordHit *record, fp_t t_min, fp_t t_max) const {
...
__device__ void Hitable_Sphere::updateBoundingBox() {
	bounding.setBounding(center - Vec3f(radius), center + Vec3f(radius));
}


// texture - mapper
__device__ Texture_Mapper_Sphere::Texture_Mapper_Sphere() {
	input_size	= 1;
...
__device__ void Texture_Mapper_Sphere::setSphere(Hitable_Sphere *sphere) {
	this->sphere = sphere;
}


__device__ void Texture_Mapper_Sphere::setPixel(const Vec3f &point, const Vec3f &pixel) {

}


__device__ void Texture_Mapper_Sphere::_getPixel_(Vec3f &dst, Vec3f *src) const {
	if (sphere == nullptr) {
		dst = Vec3f();
		return;
	}
...
// texture - direction
__device__ Texture_Direction_Sphere::Texture_Direction_Sphere() {
	input_size	= 2;
	input_list	= new Texture*[input_size];
	for (int32_t i = 0; i < input_size; i++) input_list[i] = nullptr;
}
...
__device__ void Texture_Direction_Sphere::setSphere(Hitable_Sphere *sphere) {
	this->sphere = sphere;
}


__device__ void Texture_Direction_Sphere::setPixel(const Vec3f &point, const Vec3f &pixel) {

}


__device__ void Texture_Direction_Sphere::_getPixel_(Vec3f &dst, Vec3f *src) const {
	if (sphere == nullptr) {
		dst = Vec3f();
		return;
	}

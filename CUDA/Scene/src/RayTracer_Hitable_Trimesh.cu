...
// Operation Handling
// hitable
__device__ Hitable_Trimesh::Hitable_Trimesh() {
	point[0]	= Vec3f(0);
	point[1]	= Vec3f(0);
	point[2]	= Vec3f(0);
...
__device__ void Hitable_Trimesh::setPoint(const Vec3f &p0, const Vec3f &p1, const Vec3f &p2) {
	this->point[0]	= p0;
	this->point[1]	= p1;
	this->point[2]	= p2;
...
__device__ int8_t Hitable_Trimesh::hit(RecordHit *record, fp_t t_min, fp_t t_max) const {
	...
	if (!VecMath::intersectPoint_rayPlane(result_point, result_length, ray, &plane)) return 0;

	// check if the intersection point32_t is inside the triangle
	if (!VecMath::checkInside_convexPolygon(result_point, point, 3, normal)) return 0;

	// configure hit record
	// but first check if is between t_min and t_max
	// fp_t ray_length = result_ray.length();  
	if (result_length <= t_min || result_length >= t_max) return 0;
...
__device__ void Hitable_Trimesh::updateBoundingBox() {
	bounding.setEmpty();
...
// texture - mapper
__device__ Texture_Mapper_Trimesh::Texture_Mapper_Trimesh() {
	input_size	= 1;
...
__device__ void Texture_Mapper_Trimesh::setTrimesh(Hitable_Trimesh *trimesh) {
	this->trimesh = trimesh;
}


__device__ void Texture_Mapper_Trimesh::setPixel(const Vec3f &point, const Vec3f &pixel) {

}


__device__ void Texture_Mapper_Trimesh::_getPixel_(Vec3f &dst, Vec3f *src) const {
	if (trimesh == nullptr) {
		dst = Vec3f();
		return;
	}
...
// texture - direction
__device__ Texture_Direction_Trimesh::Texture_Direction_Trimesh() {
	input_size	= 2;
	input_list	= new Texture*[input_size];
	for (int32_t i = 0; i < input_size; i++) input_list[i] = nullptr;
}
...
__device__ void Texture_Direction_Trimesh::setTrimesh(Hitable_Trimesh *trimesh) {
	this->trimesh = trimesh;
}


__device__ void Texture_Direction_Trimesh::setPixel(const Vec3f &point, const Vec3f &pixel) {

}


// TODO: not yet completed
__device__ void Texture_Direction_Trimesh::_getPixel_(Vec3f &dst, Vec3f *src) const {
	if (trimesh == nullptr) {
		dst = Vec3f();
		return;
	}

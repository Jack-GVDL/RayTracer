...
class Tracer_Ops_DLL(Tracer_Ops):
	...
	# 	# get dll
	# 	self._dll_tracer = ctypes.CDLL(os.path.join(os.getcwd()), self._file_tracer)
	...
	def Test_testUint8Array(self, array: List[bytes], size: int) -> None:
		...
		self._dll_tracer.RayTracer_Test_testUint8Array(array, size)
	...
	def Sample_buildScene(self, index: int) -> int:
		return self._dll_tracer.RayTracer_Test_buildScene(c_int(index))

	# tracer
	def Tracer_trace(self, pixel: Vec3f, x: float, y: float, depth: int) -> int:
		array_pixel = (c_double * 3)(*pixel.array)
		result: int = self._dll_tracer.RayTracer_Tracer_trace(array_pixel, c_double(x), c_double(y), c_int(depth))
	...
	def Surface_create(self, type: int) -> int:
		return self._dll_tracer.RayTracer_Surface_create(c_int(type))
	...
	def Surface_config(self, index: int, type: int, data: List[bytes], size: int) -> int:
		...
		return self._dll_tracer.RayTracer_Surface_config(c_int(int), c_int(type), array_data, c_uint32(size))

	# texture
	def Texture_create(self, type: int) -> int:
		return self._dll_tracer.RayTracer_Texture_create(c_int(type))
	...
	def Texture_config(self, index: int, type: int, data: List[bytes], size: int) -> int:
		# TODO: currently data is const uint8_t*, cannot pass out value from dll
		array_data = (c_uint8 * len(data))(*data)
		return self._dll_tracer.RayTracer_Texture_config(c_int(index), c_int(type), array_data, c_int(size))

	# scatter
	def Scatter_create(self, type: int) -> int:
		return self._dll_tracer.RayTracer_Scatter_create(c_int(type))
	...
	def Scatter_config(self, index: int, type: int, data: List[bytes], size: int) -> int:
		# TODO: currently data is const uint8_t*, cannot pass out value from dll
		array_data = (c_uint8 * len(data))(*data)
		return self._dll_tracer.RayTracer_Scatter_config(c_int(index), c_int(type), array_data, c_int(size))

	# scene object - hitable
	def SceneObject_Hitable_create(self, type: int) -> int:
		return self._dll_tracer.RayTracer_SceneObject_Hitable_create(c_int(type))
	...
	def SceneObject_Hitable_config(self, index: int, type: int, data: List[bytes], size: int) -> int:
		# TODO: currently data is const uint8_t*, cannot pass out value from dll
		array_data = (c_uint8 * len(data))(*data)
		return self._dll_tracer.RayTracer_SceneObject_Hitable_config(c_int(index), c_int(type), array_data, c_int(size))

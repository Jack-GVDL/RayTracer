import struct
from Base import Vec3f, Hitable, TracerType
...
class Hitable_Sphere(Hitable):
	...
	def setCenter(self, center: Vec3f) -> None:
		...
		result: int			= self._ops_tracer.Hitable_config(self.object_index, 0, data)

	def setRadius(self, radius: float) -> None:
		data:	bytes		= struct.pack("d", radius)
		result:	int			= self._ops_tracer.Hitable_config(self.object_index, 1, data)

	# Interface
	def start(self) -> None:
		type_index: int		= self._getType_(self._ops_tracer, "sphere")
		self._object_index	= self._ops_tracer.Hitable_create(type_index)
...
class Hitable_Trimesh(Hitable):
	...
	def setPoint_2(self, point: Vec3f) -> None:
		data:	bytes		= point.convertToBytes()
		result:	int			= self._ops_tracer.Hitable_config(self.object_index, 2, data)
...
class Hitable_AABB(Hitable):
	...
	def addHitable(self, hitable: Hitable) -> bool:
		...
		result: int = self._ops_tracer.Hitable_interact(self.object_index, 0, (hitable.object_index,), (TracerType.HITABLE,))
	...
	def rmHitable(self, hitable: Hitable) -> bool:
		...
		result: int = self._ops_tracer.Hitable_interact(self.object_index, 1, (hitable.object_index,), (TracerType.HITABLE,))
		return True if result == 0 else False
